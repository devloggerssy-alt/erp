    ...
    return (
        <form onSubmit={handleSubmit((v) => mutate(v))} className="space-y-4">
            <div className="space-y-2">
                <label className="text-sm font-medium">{t("onboarding.fiscalYear.name")}</label>
                <Input {...register("name")} placeholder={`FY ${new Date().getFullYear()}`} />
                {errors.name && <p className="text-sm text-destructive">{errors.name.message}</p>}
            </div>
            <div className="space-y-2">
                <label className="text-sm font-medium">{t("onboarding.fiscalYear.startDate")} *</label>
                <DatePickerField
            ...
            <div className="space-y-2">
                <label className="text-sm font-medium">{t("onboarding.fiscalYear.endDate")} *</label>
                <DatePickerField
