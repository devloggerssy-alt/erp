...
2. **`opening-balances.service.ts`'s JE numbering format changes from `JE00001` to `JE-00001`** once routed through the shared `AccountingPostingFacade` (Task 16), because the facade always allocates numbers via `DocumentSequencesRepository.getNextNumber`, which hyphenates, while this one call site previously incremented `documentSequence` by hand without a separator. The other five posting paths already used the hyphenated format before Phase 1 — this call site was the outlier. Not covered by any golden master (opening-balance is pinned at the pure line-math level only), so this change is silent unless someone reads this log or the decision box in Task 16. **Resolved:** user confirmed proceeding with the format change (asked explicitly before implementing Task 16, since it's a real behaviour change with no golden-master coverage) — see the commit for Task 16.

## Plan corrections discovered while executing Task 18/19

Two places where this plan's own later verification steps didn't match what its own earlier tasks (11-16) actually built. Neither is an accounting bug — both are the plan being wrong about its own state. Flagged here per Task 19 Step 5's instruction to update stale log entries before merging.

3. **Task 18's ESLint rule, as literally specified ("block all of `modules/accounting/**` except `posting`"), would have broken 6 pre-existing call sites Tasks 11-16 themselves wrote.** `invoice-posting.service.ts`, `stock-counts.service.ts`, and `inventory.service.ts` all import `assertFiscalPeriodOpen` directly and by design (see this plan's "Deviation #2" — callers fail-fast on a closed period *before* building a `PostingIntent`). `expenses.service.ts`, `stock-counts.service.ts`, `payments.service.ts`, and `invoices.service.ts` (+ their modules) import `DocumentSequencesService`/`Module` directly for their own document number (`EXP-`, `SC-`, `PAY-`, `INV-`) — unrelated to which GL account a posting hits. `onboarding.service.ts` imports `FinancialSettingsService` and `FiscalPeriodsService` to *write* initial tenant config, not to resolve accounts for posting. None of these is GL account-resolution policy, the actual thing F1 fences off. **Resolved:** user confirmed narrowing the rule's scope (asked explicitly before writing the ESLint config) to block only `accounting/accounts/{services,repositories,presenters,controllers,dto,events}/**` + `accounts.module.ts`, exempting `posting/**` (as originally specified) plus `document-sequences/**`, `financial-settings/**`, `fiscal-periods/**`, and `accounts/utils/**`. Proven via probe-import test (Task 18 Step 4) that the actual leak surface — `JournalPostingService`, `OpeningBalancesService` — is still fully blocked. See Task 18's commit.

4. **Task 19 Step 2's exact grep and the "Done when" checklist below both expect zero output for a pattern that includes `assertFiscalPeriodOpen` and `FinancialSettingsService`** — but per correction 3 above, those two are legitimately still imported outside `modules/accounting/`. The grep now intentionally returns 6 lines (see Task 19 Step 2's run). The corrected, accurate check is that `JournalPostingService`/`OpeningBalancesService` (the actual GL-policy leak surface) return zero matches outside `modules/accounting/`, which holds.

---

## Done when

- [x] Golden-master snapshots identical to their Phase 0 commit (Task 17, Step 8 — 25/25 pass)
